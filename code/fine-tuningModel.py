...
from config import is_rematch,   do_swa, just_test, use_PGD, use_ema, use_lookahead, use_sda, name, \
     use_k_fload,add_freeze,add_round_one_data
from ema import EMA
...
class BERTClass(torch.nn.Module):
    def __init__(self):
        ...
        self.l1 = transformers.AlbertForSequenceClassification.from_pretrained('../user_data/tianchiAlbert_v2',config=config)
        if add_freeze:
            print("add freeze")
        ...
        # self.l3 = torch.nn.Linear(1359, len(label_cnt))
    ...
    def forward(self, inputs):
        ...
        # print("output_1",output_1.logits.shape)

        # import pdb
        # pdb.set_trace()
        # print("output_1",output_1.size)
        # output_2 = self.l2(output_1.prediction_logits)
        # #print("output_2",output_2)
        # output = torch.sum(output_2, dim=1)
...
def train(train_dataloader, global_step=0):
    ...
    for batch, dat in enumerate(train_dataloader):
        ...
        loss = criterion(output.view(-1, len(label_cnt)), targets)
        if use_sda:  # 使用自集成
            with torch.no_grad():
